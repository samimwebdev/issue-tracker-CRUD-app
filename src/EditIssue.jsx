...
const EditIssue = ({ issues, updateIssue }) => {
  ...
  const [pickedIssue, setPickedIssue] = useState(defaultIssue)
  ...
  const handleSubmit = (evt) => {
    evt.preventDefault()
    const { title, subTitle, assignedTo, startDate, endDate } = pickedIssue
    //checking error
    if (title === '') {
    ...
    const isValid = Object.values(pickedIssue).every((elm) => elm)
    console.log(isValid)
  ...
  return (
    <>
      {pickedIssue && (
        <>
          <h1 className='mb-4 mt-4'>Edit Issue</h1>
